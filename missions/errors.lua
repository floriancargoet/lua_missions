...
function test_error_removes_file_info_if_second_param_is_0() -- in fact the previous test did the same
...
function test_assert_is_defined_by_lua()
  ...
  -- proposed solution
  local function assert_reader(boolean, message)
    return (not not boolean) or error(message, 2) -- return true if truthy (not not = toboolean() )or throw an error (level 2: caller)
  end
  status, message = pcall(function() assert_reader(false, "This is an error") end)
  assert_equal(false, status)
  assert_equal("errors.lua:70: This is an error", message)
end
